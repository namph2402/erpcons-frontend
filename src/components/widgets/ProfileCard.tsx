  ...
  return (
    <section className="profile">
      <div className="profile__identity">
        <Avatar name={user.name} src={user.avatar} size={96} status={user.status} />
        <div className="profile__info">
          <div className="profile__name-row">
            <h2 className="profile__name">{user.name}</h2>
            {tag && <Badge tone="info" size="md">{tag}</Badge>}
          </div>
      ...
      <ul className="profile__stats">
        {stats.map((s) => (
          <li key={s.id} className="profile__stat">
            <span className={`profile__stat-icon profile__stat-icon--${s.tone ?? 'info'}`}>
            ...
            <div>
              ...
              <p className="profile__stat-value num">
                {s.value}
                {s.total && <span className="profile__stat-total">/ {s.total}</span>}
              </p>
              {s.caption && <p className="profile__stat-caption">{s.caption}</p>}
            </div>
          </li>
        ))}
      </ul>
