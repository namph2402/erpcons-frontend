...
const PROJECT_STATUS = [
  { label: 'Đúng tiến độ', value: 12, color: 'var(--success)', note: '12 (50%)' },
...
export default function Dashboard({
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (taskTab === 'upcoming') return tasks.filter((t) => t.state === 'upcoming')
    return tasks
  }, [tasks, taskTab])
  ...
  const projectColumns: Column<Project & Record<string, unknown>>[] = [
    ...
    },
    {
      ...
      header: 'Trạng thái',
      width: '150px',
      render: (p) => <Badge tone={STATUS_TONE[p.status] ?? 'default'} size="md">{p.status}</Badge>,
    },
  ]
    ...
    >
      <div className="page">
        <div className="greeting">
          <div>
            <h1 className="greeting__title">Chào buổi sáng, {currentUser.name}! 👋</h1>
            <p className="greeting__subtitle">Chúc bạn có một ngày làm việc hiệu quả.</p>
          </div>
        ...
        <div className="stat-row">
          ...
          <StatCard
            ...
            icon="calendar_month"
            tone="neutral"
            trend={{ direction: 'up', value: '2 dự án', label: 'mới trong tháng' }}
          />
        </div>

        <div className="grid-main-aside">
          <Card title="Tổng quan dự án" link={{ label: 'Xem tất cả dự án' }} flush>
            <div style={{ padding: '0 var(--sp-5)' }}>
              <div className="kpi-strip">
                ...
                <div className="kpi-strip__item">
                  ...
                  <p className="kpi-strip__value kpi-strip__value--info num">18</p>
                </div>
                <div className="kpi-strip__item">
                  <p className="kpi-strip__label">Đúng tiến độ</p>
                  <p className="kpi-strip__value kpi-strip__value--info num">12</p>
                </div>
                <div className="kpi-strip__item">
                  <p className="kpi-strip__label">Chậm tiến độ</p>
                  <p className="kpi-strip__value kpi-strip__value--danger num">6</p>
                </div>
                <div className="kpi-strip__item">
                  <p className="kpi-strip__label">Hoàn thành</p>
                  <p className="kpi-strip__value kpi-strip__value--success num">6</p>
                </div>
              </div>
            </div>
        ...
        <div className="grid-1-1-1.2">
          <Card title="Dòng tiền (Cashflow)" link={{ label: 'Xem chi tiết' }}>
            <div className="mini-stat-row">
              <div className="mini-stat">
                ...
                <LineChart
                  minimal
                  height={40}
                  series={[{ name: 'in', color: 'var(--success)', points: CASHFLOW_IN, area: true }]}
                />
              </div>
              <div className="mini-stat">
                ...
                <LineChart
                  minimal
                  height={40}
                  series={[{ name: 'out', color: 'var(--danger)', points: CASHFLOW_OUT, area: true }]}
                />
              </div>
            </div>
            ...
            <div className="mini-stat" style={{ marginTop: 'var(--sp-3)' }}>
              ...
              <LineChart
                minimal
                height={70}
                series={[{ name: 'net', color: 'var(--info)', points: NET_FLOW, area: true }]}
              />
          ...
          <Card title="Biểu đồ tiến độ tổng hợp" link={{ label: 'Xem chi tiết' }}>
            <DonutChart
              data={PROJECT_STATUS}
              size={150}
              thickness={26}
